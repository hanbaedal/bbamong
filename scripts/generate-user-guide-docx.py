...
SECTIONS: list[tuple[str, list[str]]] = [
    ...
    (
        ...
        [
            ...
            "왼쪽: 빠몽이 캐릭터(탭 가능)와 「실시간 예측게임」·「친구·동호회 방」·「딜레이 예측게임」. 빠몽이와 실시간 버튼은 실시간 예측(/prediction)으로, 「딜레이 예측게임」은 자동 딜레이 예측(/delay-prediction)으로, 「친구·동호회 방」은 방 목록으로 갑니다.",
    ...
    (
        "딜레이 예측게임",
        [
            "홈의 「딜레이 예측게임」으로 들어갑니다 (/delay-prediction). 운영자 없이 실황을 보고 타석이 자동으로 열립니다.",
            "다음 스포츠에 경기가 시작된 뒤(경기 중)에만 예측 창이 열립니다. 경기 전 선발명단만으로는 타석이 열리지 않습니다.",
            "같은 경기에서 실시간 예측에 한 번이라도 참여했으면 딜레이에는 참여할 수 없습니다.",
            "공수교대·투수교체 광고는 약 40초입니다. 실시간 예측의 80초 광고와는 별개입니다.",
        ],
    ),
    (
        "예측 화면 진입",
        [
            "홈 왼쪽의 빠몽이 또는 「실시간 예측게임」으로 예측 화면(/prediction)에 들어갑니다.",
...
def main() -> None:
    ...
    table = doc.add_table(rows=1 + len(flow_rows), cols=4)
    ...
    for caption, fname in (
        ...
        ("6. 주루 — 필리스 실사 (홈런은 1·2·3루를 돌아 홈)", "scene-running.jpg"),
    ):
