...
def write_admin() -> Path:
    ...
    add_section(
        ...
        "3. 경기·예측 운영 포인트",
        [
            "경기 관리는 API-Sports 일정 동기화·구장·등록 순서를 다룹니다.",
            "실시간 모니터링에서 배팅 분포·사이드벳·수동 스코어를 확인할 수 있습니다.",
            "운영자 API 폴링 슬롯(1~5경기)을 켜야 해당 경기 연동·사이드벳이 활성화됩니다.",
        ],
    )
    footer_note(doc)
...
def write_operator() -> Path:
    doc = new_doc("빠몽이 사용 설명서 — 운영자", "PPAMONG Manager App · 경기 현장 운영")
    add_section(
        doc,
        "1. 로그인",
        [
    ...
    add_section(
        ...
        [
            ...
            "정상 흐름: 예측시작 → 예측중지 → 결과 전송 → (자동) 다음 타자",
            "3아웃이면 자동 다음타자 대신 「공수교대」를 누릅니다.",
            "투수교체: 광고 시작 / 예측시작: 광고 중지",
            "대타: 이름·타율·안타·홈런·타점·OPS 입력 → 예측 화면에 「대타가 나옵니다」 표시",
            "팀명 클릭: 주전 1~9 타순·시즌 전적 수동 입력",
        ],
    )
    add_section(
        doc,
        "3. 상태 표시",
        [
            "구장명 · 경기전 / N회 초·말 / 경기종료 / 연기됨",
            "네트워크 끊김 시 「실시간 연결 재시도 중」 안내 후 자동 재연결을 시도합니다.",
        ],
    )
    footer_note(doc)
...
def write_db() -> Path:
    ...
    add_section(
        ...
        [
            ...
            "pinchHitter — 현재 타석 대타 (다음 타자·공수교대 시 해제)",
            "sideBetsLocked / predictionEnabled — 사이드벳 마감·타석 예측 오픈",
            "liveScoreboard / controlMode — API·수동 스코어",
        ],
    )
    add_section(
        doc,
