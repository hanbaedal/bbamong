...
const homeEntrySrc = readFileSync("shared/homeGameEntry.ts", "utf8");
assert(!homeEntrySrc.includes("준비 중"), "no delay-soon copy");

const guideSrc = readFileSync("client/src/components/user/UserGuideContent.tsx", "utf8");
assert(!guideSrc.includes("딜레이는 준비"), "guide not soon");
assert(guideSrc.includes("/delay-prediction"), "guide delay path");
assert(guideSrc.includes("경기가 시작된 뒤"), "guide ongoing only");

const defaultGuide = readFileSync("shared/defaultGameGuide.ts", "utf8");
assert(!defaultGuide.includes("준비 중입니다"), "game guide not soon");
