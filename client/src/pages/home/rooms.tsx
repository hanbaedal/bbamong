...
export default function FriendRoomsPage() {
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      setRanking([]);
      await loadMine();
      const rank = await roomFetch(`/api/rooms/${data.room.id}/ranking`);
      setRanking(rank.ranking ?? []);
    } catch (e) {
  ...
  const hostName =
    detail?.hostName ||
    detail?.members.find((m) => m.role === "host")?.name ||
    "방장";
    ...
    <div className="friend-rooms-page" data-testid="friend-rooms-page">
      ...
      {!isGuest && mode === "list" ? (
        <div className="friend-rooms-split" data-testid="friend-rooms-hub">
          <div className="friend-rooms-pane">
            <div className="friend-rooms-create-hero">
              ...
              <button
                ...
                data-testid="friend-rooms-create-btn"
                onClick={() => {
          ...
          <div className="friend-rooms-pane friend-rooms-pane--scroll">
      ...
      {!isGuest && mode === "create" ? (
        <div className="friend-rooms-split" data-testid="friend-rooms-create">
          <div className="friend-rooms-pane">
            <div className="friend-rooms-form-grid">
              <label>
                방 이름
              ...
              <div className="friend-rooms-form-row">
                <label>
                  응원 팀
          ...
          <div className="friend-rooms-pane">
            <div className="friend-rooms-disclaimer-wrap">
              <h2>{FRIEND_ROOM_DISCLAIMER_TITLE}</h2>
              <div className="friend-rooms-disclaimer-scroll">
      ...
      {!isGuest && mode === "detail" && detail ? (
        <div className="friend-rooms-split" data-testid="friend-rooms-detail">
          <div className="friend-rooms-pane">
            <h2 className="friend-rooms-detail-title">{detail.name}</h2>
            <p className="friend-rooms-detail-host">방장 · {hostName}</p>
            ...
            <ul className="friend-rooms-members-inline">
            ...
            <div className="friend-rooms-actions">
              ...
              <button type="button" className="friend-rooms-btn" onClick={() => shareInvite(detail)}>
                친구초대 (링크공유)
              </button>
              {detail.isHost ? (
                <button
                  type="button"
                  className="friend-rooms-btn friend-rooms-btn--danger"
                ...
                <tbody>
                  {ranking.map((r, i) => (
                    <tr key={r.userId}>
                      ...
                      <td>{r.name}</td>
                      <td>
                        {r.hits}/{r.fails ?? Math.max(0, (r.bets ?? 0) - r.hits)}
                      </td>
                      <td>{r.successPoints ?? 0}</td>
                      <td>{r.failPoints ?? 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {ranking.length === 0 ? (
                <p className="friend-rooms-muted">오늘 정산된 예측이 없습니다.</p>
              ) : null}
            </div>
          </div>
        </div>
...
export function FriendRoomJoinPage() {
  ...
  return (
    <div className="friend-rooms-page" data-testid="friend-room-join-page">
      ...
      <div className="friend-rooms-pane" style={{ maxWidth: 480, margin: "0 auto" }}>
