...
export default function PointsRankingPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<PointsRankingData>>({
    queryKey: [`/api/rankings/points?page=${currentPage}&limit=${itemsPerPage}`],
  });
  ...
  return (
    <AdminLayout>
      <div className="flex items-center gap-2 mb-6" data-testid="breadcrumb">
        ...
        <span className="text-sm text-[#201E22]">승리 획득 포인트 랭킹</span>
      </div>
      ...
      >
        <img src={assets.adListIcon} className="w-8 h-8" alt="icon" /> 승리 획득 포인트 랭킹
      </h1>
      ...
      <div className="flex gap-8 border-b border-[#E9E9E9] mb-6">
        <button
          ...
          data-testid="tab-points-ranking"
        >
          승리 획득 포인트 랭킹
        </button>
      </div>
      ...
      <div className="flex items-center gap-4 px-4 py-3 bg-[#F9F9F9] text-sm font-medium text-[#4D4B4E] mb-2">
        ...
        <div className="flex-[15%] text-right">획득 포인트</div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32">
            <p className="text-base text-[#BFBFBF]">획득 포인트 내역이 없습니다.</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-[15%] font-medium text-[#E11936] text-right">
                    {ranking.earnedPoints?.toLocaleString() || 0}
                  </div>
                </div>
              );
            })}
