...
type DonationResponse = MemberPaginatedMeta & {
...
export default function DonationRankingsPage() {
  ...
  const { data, isLoading } = useQuery<DonationResponse>({
    queryKey: ["admin-donation-rankings", platform, currentPage, itemsPerPage],
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/admin/donation-rankings?page=${currentPage}&limit=${itemsPerPage}&platform=${platform}`,
      );
      return res.json();
    },
  });
  ...
  return (
    <AdminLayout>
      <div className="flex items-center gap-2 mb-3" data-testid="breadcrumb">
        ...
        <span className="text-xs text-[#201E22]">사회공헌참여기록 관리</span>
      </div>
          ...
          <table className={memberCompactTableClass}>
            <thead>
              <tr className="bg-[#FAFAFA] border-b border-[#E9E9E9] text-left">
                ...
                <th className={`${memberThClass} w-24 text-right`}>공헌기록</th>
              </tr>
            </thead>
            <tbody>
              {donors.map((donor, index) => {
                ...
                return (
                  <tr key={donor.id} className={memberRowClass} data-testid={`donor-row-${index}`}>
                    <td className={`${memberTdClass} text-center tabular-nums`}>{rank}</td>
                    <td className={memberTdClass} title={donor.username}>
                      {truncateText(donor.username, 18)}
                    </td>
                    <td className={memberTdClass}>{truncateText(donor.name, 8)}</td>
                    <td className={memberTdClass} title={donor.email ?? undefined}>
                      {truncateText(donor.email, 22)}
                    </td>
                    <td className={`${memberTdClass} text-right font-semibold text-[#E11936] tabular-nums`}>
                      {(donor.totalDonationAmount ?? 0).toLocaleString()}
                    </td>
                  </tr>
                );
              })}
