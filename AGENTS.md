...
- `wait_result` 중 `round_next`는 결과 생략(`skippedResult`)이 아니면 보류한다. 투수교체 환불 시 서버가 `skippedResult: true`를 보낸다. **예측을 못 고른 회원도** 같은 라운드면 결과대기 → 결과 큰 글씨 → 다음 타석(포인트·주루 없음). 복귀 `/check`는 **라운드가 바뀌었거나 이미 본 결과**일 때만 결과대기를 해제한다(같은 라운드 제출 레이스는 유지). 자리비움 중 결과는 주루를 생략하고 짧은 배너만 쓰며, 다음 타석이 이미 열려 있으면 복귀 즉시 예측 창으로 간다. 운영자 3아웃 잔상에 실황이 같은 초/말 0~2면 예측 시작·투수교체·다음타자를 막지 않는다.
...
- **표시·자동 동기화**: “N회 초/말”·팀 옆 점수는 **실황 스코어보드를 우선**한다 (`shared/matchPhaseDisplay.ts`, `shared/liveScoreTotals.ts`). 실황 폴링은 `liveScoreboard`와 **같은 초/말의 타순 인덱스**만 맞춘다. 운영자 `gameInning`/`inningHalf`/`outsInHalf`는 **공수교대 버튼**이 권위(폴링이 덮지 않음). **타석 진행은 운영자 버튼이 권위**다: 예측 시작·결과 확정·다음타자·공수교대·투수교체·대타는 수동. **자동 예측 중지**는 `schedulePredictionAutoStop` 한 루틴만(시작 후 8초). 공수교대·투수교체 **광고 80초**. **3아웃 카운트·예측 종료는 운영자 결과 누적(`outsInHalf`)이 권위**다(2아웃+아웃 / 1아웃+병살 / 노아웃+삼살). **공수교대 시점**은 네이버: 같은 초/말 3아웃이면 넘기고, 실황이 이미 다음 초/말(0~2아웃)이면 **공수교대로 운영자 초/말을 실황에 맞추고 광고**. 실황 아웃이 없으면(위젯 공란) 가짜 0으로 막지 않는다. 같은 초/말에서 실황이 늦으면 공수교대 두 번(강제). **자동 공수교대(광고)는 없다.** 실황은 제안만 (`auto_result_suggested` / `auto_action_suggested`). `liveAutoOperator.ts`는 실황 동기화·힌트·8초 중지. 예전 `liveAutoEnabled=false` 잔여값은 폴링 시 자동 복구. **예측 시작 시 광고 자동 중지**. 경기종료 10초 연출 후 로그아웃. 운영자 공수교대는 `liveScoreboard` 이닝을 덮어쓰지 않는다.
