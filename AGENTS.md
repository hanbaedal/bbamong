...
- **표시·자동 동기화**: “N회 초/말”·팀 옆 점수는 **실황 스코어보드를 우선**한다 (`shared/matchPhaseDisplay.ts`, `shared/liveScoreTotals.ts`). 실황 폴링 시 운영자 `gameInning`/`inningHalf`/`outsInHalf`/`batterIndexInHalf`도 실황에 맞춘다. `server/liveMatch/liveAutoOperator.ts`는 **타석 상태머신**(`shared/atBatPhase.ts`: 대기→예측열림→예측닫힘→결과확정→다음/교대)만 따라 전이한다. 가드: 타자명 3초·투수명 5초 안정화, 결과 전 다음타자/공수/광고/투수교체 금지, 아웃(아웃수↑)·홈런·**1루(1루타·포볼·데드볼, 아웃 유지+타자 교체)** 자동 확정·그 외는 제안. 실황 타자가 선발과 다르면 **대타 표시·자동 pinch 설정**. 투수교체는 같은 타석이므로 대타를 지우지 않는다. `liveAutoEnabled`(기본 ON) OFF면 액션 없이 표시 동기화만. 수동 버튼은 동일 단계로 오버라이드(`at_bat_phase` WS). **예측 시작 시 광고 자동 중지**. 경기종료는 약 10초 「경기종료」연출 후 로그아웃(자격 회수는 지연). 운영자 공수교대는 `liveScoreboard` 이닝을 덮어쓰지 않는다. 진행 위젯은 TV 실황(다음/네이버)을 쓴다.
