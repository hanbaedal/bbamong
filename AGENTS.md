...
- **표시·자동 동기화**: “N회 초/말”·팀 옆 점수는 **실황 스코어보드를 우선**한다 (`shared/matchPhaseDisplay.ts`, `shared/liveScoreTotals.ts`). 실황 폴링 시 운영자 `gameInning`/`inningHalf`/`outsInHalf`/`batterIndexInHalf`도 실황에 맞춘다. `server/liveMatch/liveAutoOperator.ts`는 **타석 상태머신**(`shared/atBatPhase.ts`: 대기→예측열림→예측닫힘→결과확정→다음/교대)만 따라 전이한다. 가드: 타자명 **2초**·투수명 **3초** 안정화(env로 조정 가능), 예측 열림 기본 **10초** 후 자동 중지, 실황 폴링 기본 **2초**(최소 1.5초). 결과 전 다음타자/공수/광고/투수교체 금지, 아웃(아웃수↑)·1~3루·홈런(아웃 유지+타자 교체) 자동 확정·희생/병살→아웃·야수선택→1루. 애매하면 제안+운영자 **1탭 확정**(`shared/operatorNextAction.ts`「지금」안내). 실황 타자≠선발이면 **대타 표시·자동 pinch**. 투수교체는 대타 유지. **`liveAutoEnabled`(기본 ON)=완전 자동** — 운영자 없이도 타석 전이. **OFF=반자동** — 점수·초/말·주자 등 **표시는 실황 동기화 유지**, 예측 시작/결과/다음/공수/투수는 운영자. TV를 보며 운영할 때는 반자동이 체감 지연이 적다(자동은 다음/네이버 피드+가드 지연). **예측 시작 시 광고 자동 중지**. 경기종료 10초 연출 후 로그아웃. 운영자 공수교대는 `liveScoreboard` 이닝을 덮어쓰지 않는다.
