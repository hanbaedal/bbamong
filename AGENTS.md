...
- **표시·자동 동기화**: “N회 초/말”·팀 옆 점수는 **실황 스코어보드를 우선**한다 (`shared/matchPhaseDisplay.ts`, `shared/liveScoreTotals.ts`). 실황 폴링 시 운영자 `gameInning`/`inningHalf`/`outsInHalf`/`batterIndexInHalf`도 실황에 맞춘다. `server/liveMatch/liveAutoOperator.ts` — 타자 변경 시 예측 시작 후 15초 중지, 3아웃·초말 전환 시 라운드 진행+광고(90초 쿨다운), 투수명 변경 시 투수교체+광고, 문자중계 아웃/홈런은 자동 결과·그 외·대타는 운영자 확인 제안. 수동 버튼은 오버라이드로 유지. 운영자 공수교대는 `liveScoreboard` 이닝을 덮어쓰지 않는다. 진행 위젯은 TV 실황(다음/네이버)을 쓴다.
