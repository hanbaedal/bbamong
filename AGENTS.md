...
- `wait_result` 중 `round_next`는 결과 생략(`skippedResult`)이 아니면 보류한다. 투수교체 환불 시 서버가 `skippedResult: true`를 보낸다. **예측을 못 고른 회원도** 같은 라운드면 결과대기 → 결과 큰 글씨 → 다음 타석(포인트·주루 없음). 복귀 `/check`는 **라운드가 바뀌었거나 이미 본 결과**일 때만 결과대기를 해제한다(같은 라운드 제출 레이스는 유지). 자리비움 중 결과는 주루를 생략하고 짧은 배너만 쓰며, 다음 타석이 이미 열려 있으면 복귀 즉시 예측 창으로 간다.
